...
def extract_coordinates_all(file):
    ...
    if not forces_present:
        return (
            np.array(
                [
                    np.array(x_dump, dtype=float),
                    np.array(y_dump, dtype=float),
                    np.array(z_dump, dtype=float),
                ]
            ),
            None,
            BOX,
        ...
        return (
            ...
            [
                np.array(fx_dump, dtype=float),
                np.array(fy_dump, dtype=float),
                np.array(fz_dump, dtype=float),
            ],
            BOX,
            symbols,
        )
...
def to_ase_atoms(coords, symbol, box, forces, charges):
    print(
        "guessing atom - if you have Elements with multiple letters or atomnames not starting with the chemical symbol please correct it afterwards"
    )
    using_forces = False
    using_charges = False
    ...
    if not using_charges:
        a = Atoms(
            [Atom(symbol=s, position=p) for s, p in zip(symbols_guessed, coords)],
            cell=mycell,
            pbc=True,
        )
        ...
        a = Atoms(
            [
                Atom(symbol=s, position=p, charge=c)
                for s, p, c in zip(symbols_guessed, coords, charges)
            ],
            cell=mycell,
            pbc=True,
        )
...
def extract_coordinates_parallel(args):
    ...
    coords, forces, box, symbol = extract_coordinates_all(
        split_trj_gro(grofile, splitstring, frame)
    )
    return to_ase_atoms(coords, symbol, box, forces, charges)


def extract_all_coordinates_from_gro_file(
    grofile: str, itpfile: str | None, processes: int = 3
):
    results_list = []
    """
