    ...
    with open(self.gro_file, 'r') as gro:
        ...
        while frame_start < len(lines):
            ...
            if frame_count % self.every_nth_frame == 0:
                frame_dir = os.path.join(self.output_dir, f"frame_{frame_count // self.every_nth_frame}")
                ...
                orcacommand_full="! "+self.method_and_basisset+" EnGrad\n%PAL NPROCS "+str(self.pal)+" END  \n\n* xyz "+self.charge+" "+self.multiplicity+" \n"
                ...
                if self.run_orca:
                    ...
                    run_orca_on_file(orca_bin,os.path.join(frame_dir, "input_all.inp"), os.path.join(frame_dir, "orca_output.txt"))
                ...
                for residue_id, atoms in residues.items():
                    ...
                    with open(os.path.join(residue_dir, "input.inp"), 'w') as inp:
                        orcacommand="! "+self.method_and_basisset+" EnGrad\n%PAL NPROCS "+str(self.pal)+" END  \n\n* xyz "+self.charge+" "+self.multiplicity+" \n"
...
#                        inp.write("! B3LYP def2-TZVP D3BJ EnGrad\n\n* xyz 0 1\n")
                        ...
                        inp.write("*")
                    if self.run_orca:
                        run_orca_on_file(orca_bin,os.path.join(residue_dir, "input.inp"), os.path.join(residue_dir, "orca_output.txt"))
